...
Changelog
---------
2026-04-20  修复加粗/斜体末尾标点后多余空格导致 ** 渲染失败的问题
...
def _convert_text(text: str) -> str:
    ...
    # e) Remove space just before a closing bold/italic marker caused by punctuation conversion.
    #    Lookbehind restricts to chars we add trailing spaces after, avoiding false positives
    #    on pre-existing spaces before opening markers.
    text = re.sub(r"(?<=[,:;!?)\]]) +(\*{1,3})(?![*\w])", r"\1", text)
    # f) Remove space just after an opening bold/italic marker before an opening bracket
    #    (Chinese bracket conversion adds a leading space: （ → ` (`).
